...
export function CourseCard({ course }: { course: any }) {
  ...
  return (
    <Link href={`/course/${course.id}`}
      ...
      <div className="relative aspect-video overflow-hidden bg-purple-900">
      ...
      <div className="flex flex-col flex-1 p-4 gap-2">
        ...
        {course.short_description && <p className="text-xs text-purple-300 line-clamp-2">{course.short_description}</p>}
        <div className="flex items-center gap-3 text-xs text-purple-400 mt-auto pt-2 border-t border-white/5">
        ...
        {course.seller?.full_name && <p className="text-[10px] text-purple-500">por <span className="text-purple-300">{course.seller.full_name}</span></p>}
      </div>
    </Link>
  )
}
