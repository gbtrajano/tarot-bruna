  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#1a0640' }}>
      ...
      <section className="relative pt-24 pb-16 px-4 text-center">
        <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at center, rgba(124,58,237,0.35) 0%, transparent 70%)' }} />
      ...
      <section className="max-w-7xl mx-auto px-4 pb-24">
        <div className="mb-8">
          <h2 className="text-xl font-display font-semibold text-white">
            {courses.length > 0 ? `${courses.length} curso${courses.length !== 1 ? 's' : ''} disponíve${courses.length !== 1 ? 'is' : 'l'}` : 'Em breve'}
          </h2>
          <p className="text-purple-300 text-sm mt-0.5">Encontre o curso ideal para sua jornada</p>
        </div>

        {courses.length === 0 ? (
          <div className="text-center py-20">
            <div className="text-6xl mb-4">🔮</div>
            <p className="text-purple-300 text-lg">Os primeiros cursos chegam em breve...</p>
          </div>
        ) : (
